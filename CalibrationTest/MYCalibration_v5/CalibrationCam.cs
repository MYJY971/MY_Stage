    ...
    class CalibrationCam : ClassicCam
    {
        ...
        private void DrawNewPoints()
        {
            if(_newPoints3D.Count>0)
            {
                ...
                {
                    GL.Vertex3(new Vector3(point.X,point.Y,-point.Z));
                }
                GL.End();
        ...
        public override void Correction(Camera surface, out float yVal, out float zVal)
        {
            if (surface._isCalibrated && _listPoints.Count==4)
            {
                ...
                foreach(Vector3 point in _listPoints)
                {
                    ...
                    yValues.Add(newPoint.Y);
                    zValues.Add(newPoint.Z);
